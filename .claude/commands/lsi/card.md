...
**Canonical source:** [ticket-card-info.md](../../docs/workflows/ticket-card-info.md) · [git-trello.md](../../overlays/lsi/docs/sdlc/git-trello.md) · [`overlays/lsi/docs/workflows/openspec-git-integration.md`](../../overlays/lsi/docs/workflows/openspec-git-integration.md) (when present)
...
6. **Run `git ts`**
   ...
   - **`git ts` is a local Git alias** (`.git-trello/bin/git-trello start`) — run it exactly as shown. **Do not** run `git-ts`, `which git-ts`, or search for a hyphenated binary ([git-trello.md](../../overlays/lsi/docs/sdlc/git-trello.md)).
   - Feed the prepared type, title, and description when `git ts` prompts interactively.
   - If `git ts` fails (missing `~/.trello_secrets`, hook error), stop and point to [git-trello.md](../../overlays/lsi/docs/sdlc/git-trello.md) install steps — do not fall back to manual `git checkout -b`.
