...
- [ ] 1.1 Add `overlays/lsi/agent-stack/commands/lsi-help.md` — **help session guardrails block at top** (per `design.md` command outline); overview template, AskQuestion menu ids, per-section templates, SDLC mermaid, bundle-path map, session loop guardrails, GitHub URL builder; **`status`**: no standing `TITLE_PREFIX` line — conditional token note only when card setup is suggested (`design.md` §9)
