# adopt-doc-link-resolution

## Purpose

Ensure adopted `.lsi/workflows/` output has no broken relative links, bundle sources enforce a three-tier link policy, and regression tests catch maintainer-path drift before adopter re-sync or `VERSION` bumps.
